/**
    ...
    Modified by Tom von Clef, September 2013
*/    
//#include <debug.js>
/** @constructor */
...
var globalConsole;
/** @constructor */
function Console( scale )
{
    this.dpiScale = (scale === 2) ? 2 : 1;

    var canvas = document.createElement("canvas");
    this.canvas = canvas;
    this.canvas.width = 640 * this.dpiScale;
    this.canvas.height = 400 * this.dpiScale;
...
Console.prototype = {
    ...
    print: function( str )
    {
        ...
        var that = this;
        var printChar = function( ch ) {
            that.ctx.fillRect( that.x * that.charWidth, 
                that.y * that.charHeight, that.charWidth, that.charHeight );
            that.ctx.drawImage( that.charImg,
                 that.charWidth * (ch % 16),
                 that.fgcolourNum * that.charHeight * 16 +
                 that.charHeight * Math.floor( ch / 16 ),
                 that.charWidth,
                 that.charHeight,
                 that.x * that.charWidth, 
                 that.y * that.charHeight, 
                 that.charWidth, 
                 that.charHeight );
            that.x += 1;
            if ( that.x == that.cols ) {
